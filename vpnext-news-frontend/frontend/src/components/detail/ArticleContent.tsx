...
const ArticleContent = memo(function ArticleContent({
  ...
  const closeTooltip = useCallback(() => setTooltipData(null), []);
  ...
  return (
    <article className="flex-1 min-w-0 max-w-[720px] mx-auto">
      ...
      {finalImage && (
        <figure className="mb-8 overflow-hidden rounded-[16px] border border-[#E4DDD3] bg-[#F3F0EB] shadow-[0_4px_20px_rgba(22,19,17,0.06)] aspect-video">
          <img
            src={finalImage}
            alt="뉴스 메인 사진"
      ...
      {aiSummary && (
        <div className="mb-7 px-5 py-4 rounded-[16px] bg-[#EFF6FF] border border-[#BFDBFE] shadow-[0_2px_10px_rgba(30,58,95,0.04)]">
          <p className="text-[11px] font-black uppercase tracking-widest mb-2.5 flex items-center gap-1.5 text-[#1A55A8]">
            <svg
          ...
          <div className="text-[15px] leading-[1.85] text-[#1E3A5F] font-sans font-medium">
            {parsedSummary}
          </div>
        </div>
      )}

      <div className="border-t border-[#E4DDD3] pt-7" onClick={handleArticleClick}>
        {news.content ? (
          <ArticleParagraphs
          ...
          <div className="p-10 text-center flex flex-col items-center gap-4 bg-[#F0F9FF] border border-[#BAE6FD] rounded-[16px]">
            <svg
              className="w-10 h-10 text-sky-500 mb-1"
              fill="none"
              stroke="currentColor"
              viewBox="0 0 24 24"
              aria-hidden="true"
            ...
            <h3 className="text-lg font-bold text-[#161311]">
              본문이 아직 수집되지 않았습니다
            </h3>
            <p className="text-[15px] text-[#5C5853] leading-relaxed">
