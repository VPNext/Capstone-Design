...
export const replaceEnglishSourceNames = (text: string): string => {
  ...
  return text
    ...
    .map((part) => {
      ...
      return part.replace(
        ENGLISH_SOURCES_REGEX,
        (match) => SOURCE_NAME_MAP[match.toLowerCase()] || match,
      );
    })
    .join("");
};
...
export const parseAndRenderSummary = (
  ...
  cleanText = cleanText.replace(parseLinkRegex, (_, linkText, rawUrls) => {
    ...
    const shouldRemove =
      /교차\s*분석용|참조\s*기사|참조\s*뉴스|하이퍼링크/i.test(trimmedText) ||
      urls.length >= 2;
  ...
  cleanText = cleanText
    ...
    .replace(
      /\b(?:네이버\s*(뉴스)?|naver|daum|다음\s*(뉴스)?|google|구글)\s*(?:등)?\s*(?:의)?\s*(?:보도\s*(내용)?|기사)?\s*(?:와|과)?\s*(?:비교하여|대조하여|비교해\s*보면|비교했을\s*때|비교해\s*보았습니다)[,.\s]*/gi,
      "",
    )
    .replace(/(?:참조\s*기사|참조\s*기사\s*하이퍼링크)[\s:]*/gi, "")
    .replace(/[\s\n]*\((?:참조\s*기사\s*)?하이퍼링크.*?\)/gi, "")
    .replace(/[\s\n]*\((?:참조\s*기사\s*)?하이퍼링크.*?$/gi, "")
    .replace(/[\s\n]*[(\[]?\s*(?:참조|출처|관련\s*(?:기사|뉴스|정보))\s*:\s*[\s\S]*$/gi, "") // "참조:", "출처:", "관련 기사:" 관련 정제 추가
    .replace(/(?:관련\s*|교차\s*분석\s*|비교\s*)?(?:정보|기사|뉴스)는\s*[\s\S]*?참조\s*(?:하여\s*작성되었습니다|하였습니다|했습니다|합니다)\.?/gi, ""); // "관련 정보는 ... 참조하였습니다" 정제 추가
  ...
  const tempLinks = Array.from(linksMap.entries()).map(([url, text]) => ({
    url,
    text,
  }));
