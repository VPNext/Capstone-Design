...
// ─── 검색어 하이라이트 ────────────────────────────────────────────────────────
const HighlightText = ({ text, keyword }: { text: string; keyword: string | null }) => {
    ...
    <>
      {parts.map((part, index) =>
        part.toLowerCase() === keyword.toLowerCase() ? (
          <mark key={index} className="bg-amber-300 text-slate-900 rounded-sm px-0.5 font-bold">
            {part}
          </mark>
        ) : (
          part
...
// ─── 신뢰도 배지 ──────────────────────────────────────────────────────────────
const CredibilityBadge = ({ label, score }: { label: string | null; score: number | null }) => {
...
const ScoreMeter = ({ score }: { score: number | null }) => {
  ...
  return (
    <div className="flex items-center gap-2 shrink-0">
      ...
      <span className="text-[11px] font-black tabular-nums" style={{ color: textColor }}>
        {pct}%
      </span>
    </div>
  );
...
const SkeletonCard = ({ featured = false }: { featured?: boolean }) => (
  <div
    ...
    style={{ borderColor: "#E4DDD3", boxShadow: "0 1px 8px rgba(22,19,17,0.06)" }}
  >
    {featured ? (
      <>
...
export default function AnalyzedNewsPage() {
  ...
  const [newsList, setNewsList]             = useState<NewsItem[]>(() => loadCache("newsList", []));
  const [page, setPage]                     = useState<number>(() => loadCache("page", 1));
  const [hasMore, setHasMore]               = useState<boolean>(() => loadCache("hasMore", true));
  const [selectedSource, setSelectedSource] = useState<string>(() => loadCache("selectedSource", "전체"));
  ...
  const fetchNews = async (pageNumber: number, sourceName: string) => {
    try {
      ...
      const response    = await api.get(`/api/news?page=${pageNumber}${sourceParam}`);
      const newItems    = response.data.items || [];

      // is_analyzed === true 필터링 (원본 유지)
      const analyzedItems = newItems.filter((item: NewsItem) => item.is_analyzed === true);

      if (newItems.length === 0) {
        setHasMore(false);
      } else {
        setNewsList((prev) => (pageNumber === 1 ? analyzedItems : [...prev, ...analyzedItems]));
      }
    } catch (err) {
  ...
  useEffect(() => {
    if (isFilterFirstRun.current) { isFilterFirstRun.current = false; return; }
    setNewsList([]);
    setPage(1);
    setHasMore(true);
    setAutoLoadCount(0);
  ...
  useEffect(() => {
    if (isPageFirstRun.current) { isPageFirstRun.current = false; return; }
  ...
  // ─────────────────────────────────────────────────────────────────────────
  return (
    <div className="flex flex-col mt-8" style={{ fontFamily: "'Noto Sans KR', sans-serif" }}>

      {/* ─── 페이지 헤더 배너 ────────────────────────────────────── */}
      <div className="mb-8">
        {/* 상단 배너 */}
        <div
          className="relative overflow-hidden rounded-3xl mb-6 px-8 py-10"
          style={{
            background: "linear-gradient(135deg, #0C1F3F 0%, #1A3A6B 50%, #0C1F3F 100%)",
            boxShadow: "0 8px 40px rgba(12,31,63,0.25)",
          }}
        >
          {/* 배경 장식 */}
          <div
          ...
          <div className="relative flex flex-col md:flex-row md:items-center justify-between gap-6">
            <div>
              ...
              >
                {keyword ? `"${keyword}" 분석 뉴스` : "AI 분석 완료 뉴스"}
              </h1>
              <p
                className="text-sm leading-relaxed"
                style={{ color: "rgba(255,255,255,0.42)", fontFamily: "'Noto Sans KR', sans-serif" }}
              >
                AI 분석이 완료된 뉴스만 모아 보여 드려요.
                <br className="hidden sm:block" />
                신뢰도 점수, AI 요약, 핵심 인물 정보를 한눈에 확인하세요.
              </p>
            </div>
        ...
        >
          <div>
            <p
              ...
              style={{ color: "#1A55A8", letterSpacing: "0.18em" }}
            >
          ...
          {SOURCES.map((src) => {
            ...
            return (
              <button
                ...
                style={
                  isActive
                    ? { background: "#0C1F3F", color: "#fff", borderColor: "#0C1F3F", boxShadow: "0 2px 8px rgba(12,31,63,0.25)" }
                    : { background: "#fff", color: "#5C5853", borderColor: "#E4DDD3" }
                }
                onMouseEnter={(e) => {
                  if (!isActive) {
                    (e.currentTarget as HTMLElement).style.borderColor = "#0C1F3F";
        ...
        >
          ...
          <div>
            <p className="text-lg font-black mb-2" style={{ color: "#2C2926" }}>
              해당 필터에 분석 완료된 뉴스가 없습니다
            </p>
            <p className="text-sm" style={{ color: "#9C9891" }}>
              메인 홈에서 일반 뉴스를 선택해 AI 분석을 직접 가동해 보세요!
            </p>
          </div>
          <Link
            to="/"
                ...
                >
                  <Link to={`/news/${news.id}`} className="block">
                    <article
                      ...
                      onMouseEnter={(e) => {
                        (e.currentTarget as HTMLElement).style.boxShadow = "0 12px 48px rgba(22,19,17,0.16)";
                        (e.currentTarget as HTMLElement).style.borderColor = "rgba(26,85,168,0.3)";
                      }}
                      onMouseLeave={(e) => {
                        (e.currentTarget as HTMLElement).style.boxShadow = "0 2px 20px rgba(22,19,17,0.08)";
                        (e.currentTarget as HTMLElement).style.borderColor = "#E4DDD3";
                      }}
                    >
                      {/* 히어로 이미지 */}
                      {displayImage ? (
                        <div className="relative h-64 sm:h-80 md:h-96 overflow-hidden">
                          ...
                          <div
                            ...
                            style={{ background: "linear-gradient(to top, rgba(12,31,63,0.85) 0%, rgba(12,31,63,0.15) 55%, transparent 100%)" }}
                          />
                          <div className="absolute top-4 left-5 flex items-center gap-2 flex-wrap">
                            <span className={`${badgeClass} text-[11px] font-black px-3 py-1.5 rounded-full`}>
                              {sourceName}
                            </span>
                            {/* AI 분석 완료 표시 */}
                            <span
                          ...
                          <div className="absolute bottom-5 left-5 right-5">
                            <h2
                              className="text-2xl sm:text-3xl font-black text-white line-clamp-2 leading-snug mb-3"
                              style={{ fontFamily: "'Noto Serif KR', serif", textShadow: "0 2px 8px rgba(0,0,0,0.4)" }}
                            >
                        ...
                        <div className="px-8 py-10" style={{ background: "#0C1F3F" }}>
                      ...
                      <div className="p-6 md:p-8">
                        <span
                          className="text-xs" style={{ color: "#9C9891" }}
                        >
                          {news.published_at?.split("T")[0]}
                        </span>
                        {displaySummary && (
                          <p className="text-[15px] leading-relaxed line-clamp-2 mt-2" style={{ color: "#3C3A36" }}>
                            <span
                              className="inline-flex items-center gap-1 text-[10px] font-black text-white rounded mr-2 align-middle"
                              style={{ background: "#1A55A8", padding: "2px 7px" }}
                            >
                              ✨ AI
                            </span>
              ...
              >
                <Link to={`/news/${news.id}`} className="block">
                  <article
                    ...
                    style={{ border: "1px solid #E4DDD3", boxShadow: "0 1px 6px rgba(22,19,17,0.05)" }}
                    onMouseEnter={(e) => {
                      (e.currentTarget as HTMLElement).style.boxShadow = "0 6px 24px rgba(22,19,17,0.12)";
                      (e.currentTarget as HTMLElement).style.borderColor = "rgba(26,85,168,0.3)";
                    }}
                    onMouseLeave={(e) => {
                      (e.currentTarget as HTMLElement).style.boxShadow = "0 1px 6px rgba(22,19,17,0.05)";
                      (e.currentTarget as HTMLElement).style.borderColor = "#E4DDD3";
                    }}
                  >
                    {/* 왼쪽 — AI 분석 인디케이터 바 */}
                      ...
                      style={{
                        background:
                          news.credibility_score != null && news.credibility_score >= 0.7
                            ? "#10B981"
                            : news.credibility_score != null && news.credibility_score >= 0.4
                            ? "#F59E0B"
                            : "#EF4444",
                      }}
                    />
                    ...
                    <div className="flex-1 p-4 sm:p-5 flex flex-col justify-between min-w-0">
                      <div>
                        <div className="flex items-center flex-wrap gap-2 mb-2.5">
                          <span className={`${badgeClass} text-[10px] font-black px-2.5 py-1 rounded-full`}>
                            {sourceName}
                          </span>
                          <span className="text-[11px]" style={{ color: "#9C9891" }}>
                            {news.published_at?.split("T")[0]}
                          </span>
                          <CredibilityBadge label={news.credibility_label} score={news.credibility_score} />
                          <ScoreMeter score={news.credibility_score} />
                        ...
                        {displaySummary && (
                          <p className="text-[13px] leading-relaxed line-clamp-2" style={{ color: "#6B6562" }}>
                            <span
                              className="inline-flex items-center gap-1 text-[9px] font-black text-white rounded mr-1.5 align-middle"
                              style={{ background: "#1A55A8", padding: "2px 5px" }}
                            >
                              AI
                            </span>
                            <HighlightText text={displaySummary} keyword={keyword} />
                    ...
                    {/* 오른쪽 이미지 */}
                    {displayImage && (
                      <div className="w-28 sm:w-40 shrink-0 overflow-hidden relative" style={{ minHeight: "110px" }}>
                        <img
                          src={displayImage}
                          alt="뉴스 썸네일"
      ...
      {/* ─── 기사 더보기 버튼 (하이브리드 무한 스크롤 — 원본 로직 유지) ─── */}
        ...
        <div className="flex justify-center mt-8 mb-4">
          <button
            ...
            onMouseEnter={(e) => {
              ...
              (e.currentTarget as HTMLElement).style.boxShadow = "0 8px 28px rgba(12,31,63,0.25)";
            }}
            onMouseLeave={(e) => {
              (e.currentTarget as HTMLElement).style.background = "#fff";
              (e.currentTarget as HTMLElement).style.color = "#0C1F3F";
              (e.currentTarget as HTMLElement).style.boxShadow = "0 2px 8px rgba(12,31,63,0.1)";
            }}
          >
            분석 뉴스 더 불러오기
            <svg
      ...
      {!hasMore && newsList.length > 0 && (
        <div className="py-10 flex flex-col items-center gap-3" style={{ color: "#9C9891" }}>
          <div className="flex items-center gap-3">
            <div className="h-px w-16" style={{ background: "#E4DDD3" }} />
            <span className="text-sm font-medium">모든 분석 뉴스를 불러왔습니다</span>
            <div className="h-px w-16" style={{ background: "#E4DDD3" }} />
          </div>
        </div>
      )}
